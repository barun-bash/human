...
func generateDockerProd(app *ir.Application) string {
	...
	b.WriteString("# Generated by Human compiler — Docker Production\n")
	...
	if hasDatabase(app) {
		...
		b.WriteString(fmt.Sprintf("    \"%s://%s:${var.db_password}@%s-db-%s:%s/%s\",\n",
			db, "postgres", name, "${var.environment}", port, appNameSnake(app)))
	}
