...
func generatePage(page *ir.Page, app *ir.Application) string {
	...
	for _, a := range page.Content {
		if a.Type == "loop" {
			ref := extractComponentRef(a.Text)
			if ref != "" {
				usedComponents[ref] = true
			} else {
	...
	if needsSuccess {
		b.WriteString("  let success = $state('');\n")
	}
	if needsError {
	...
	for _, a := range page.Content {
		...
		if a.Type == "loop" {
			loopRendered = true
			writeLoopSvelte(&b, a.Text, "  ", ctx, loopFields)
			continue
		}
...
func writeTemplateAction(b *strings.Builder, a *ir.Action, indent string, ctx *pageContext) {
	switch a.Type {
	case "display":
		writeDisplaySvelte(b, a.Text, indent, ctx)
	case "input":
		writeInputSvelte(b, a.Text, indent, ctx)
	case "interact":
		writeInteractSvelte(b, a.Text, indent, ctx)
	case "condition":
		writeConditionSvelte(b, a.Text, indent, ctx)
	case "loop":
		writeLoopSvelte(b, a.Text, indent, ctx, nil)
	case "query":
		// handled by $effect
	default:
...
func writeDisplaySvelte(b *strings.Builder, text string, indent string, ctx *pageContext) {
	cleaned := text
	...
	if strings.Contains(lower, "button") {
		...
		if strings.Contains(lower, "create") || strings.Contains(lower, "new") {
			fmt.Fprintf(b, "%s<button class=\"fab\" onclick={() => showForm = true}>+ %s</button>\n", indent, label)
		} else {
	...
	if strings.Contains(lower, "rich text") {
		expr := resolveFieldExpr(cleaned, ctx)
		if expr != "null" {
			fmt.Fprintf(b, "%s<div class=\"rich-text\">{@html %s}</div>\n", indent, expr)
		} else {
	...
	if strings.Contains(lower, "link") {
		...
		fmt.Fprintf(b, "%s<a href=\"%s\" class=\"link\">%s</a>\n", indent, target, label)
		return
	}
	...
	if strings.Contains(lower, "list of") || strings.Contains(lower, "list ") {
		fmt.Fprintf(b, "%s<!-- %s — rendered by {#each} below -->\n", indent, text)
		return
	}
...
func writeInputSvelte(b *strings.Builder, text string, indent string, ctx *pageContext) {
	lower := strings.ToLower(text)

	if strings.Contains(lower, "search") {
		fmt.Fprintf(b, "%s<input type=\"search\" placeholder=\"Search...\" class=\"search-input\" oninput={() => {/* TODO: filter */}} />\n", indent)
		return
	}
	...
	if strings.Contains(lower, "date") && (strings.Contains(lower, "picker") || strings.Contains(lower, "range")) {
		fmt.Fprintf(b, "%s<input type=\"date\" class=\"date-filter\" onchange={() => {/* TODO: filter */}} />\n", indent)
		return
	}
	if strings.Contains(lower, "button") && (strings.Contains(lower, "create") || strings.Contains(lower, "new") || strings.Contains(lower, "add")) {
		label := "New"
	...
	if strings.Contains(lower, "form") {
		writeFormSvelte(b, text, indent, ctx)
		return
	}
...
func writeLoopSvelte(b *strings.Builder, text string, indent string, ctx *pageContext, fields []string) {
	dataVar := ctx.varName
	if dataVar == "" {
		dataVar = "data"
	}
	...
	if compRef == "" {
		// Also check for PascalCase last word
		words := strings.Fields(text)
		if len(words) > 0 {
			lastWord := words[len(words)-1]
			if len(lastWord) > 0 && unicode.IsUpper(rune(lastWord[0])) {
				compRef = lastWord
			}
		}
	}
...
func writeConditionSvelte(b *strings.Builder, text string, indent string, ctx *pageContext) {
	...
	if strings.Contains(lower, "while loading") || strings.Contains(lower, "is loading") {
		if strings.Contains(lower, "skeleton") {
			...
			fmt.Fprintf(b, "%s    {#each [1,2,3] as i}\n", indent)
	...
	if strings.Contains(lower, "not logged in") || strings.Contains(lower, "is not logged in") {
		...
		writeConditionButtonsSvelte(b, content, indent+"    ", ctx)
		fmt.Fprintf(b, "%s  </div>\n", indent)
		fmt.Fprintf(b, "%s{/if}\n", indent)
		return
	}
...
func writeConditionButtonsSvelte(b *strings.Builder, content string, indent string, ctx *pageContext) {
	...
	if strings.Contains(lower, "login") && strings.Contains(lower, "signup") {
		fmt.Fprintf(b, "%s<button onclick={() => goto('/login')}>Log In</button>\n", indent)
		fmt.Fprintf(b, "%s<button onclick={() => goto('/sign-up')}>Sign Up</button>\n", indent)
		return
	}

	if strings.Contains(lower, "form") {
		writeFormSvelte(b, content, indent, ctx)
		return
	}
...
func writeInteractSvelte(b *strings.Builder, text string, indent string, ctx *pageContext) {
	lower := strings.ToLower(text)
	...
	if strings.Contains(lower, "clicking") || strings.Contains(lower, "click") {
		...
		if strings.Contains(lower, "opens a form") || strings.Contains(lower, "open a form") {
			fmt.Fprintf(b, "%s<button onclick={() => showForm = true}>%s</button>\n", indent, capitalize(label))
			return
		}
...
func resolveFieldExpr(text string, ctx *pageContext) string {
	...
	// inside {#each} blocks. Display actions outside loops must use other sources.
