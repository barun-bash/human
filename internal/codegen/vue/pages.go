...
func generatePage(page *ir.Page, app *ir.Application) string {
	...
	for _, comp := range detectUsedComponents(page) {
		fmt.Fprintf(&b, "import %s from '../components/%s.vue';\n", comp, comp)
	}
	...
	if needsFormState {
		b.WriteString("const showForm = ref(false);\n")
	}
	if needsSuccess {
		b.WriteString("const success = ref('');\n")
	}
	if needsError {
...
func writePageActionVue(b *strings.Builder, a *ir.Action, indent string, ctx *pageContext) {
	switch a.Type {
	case "display":
		writeDisplayVue(b, a.Text, indent, ctx)
	case "input":
		writeInputVue(b, a.Text, indent, ctx)
	case "interact":
		writeInteractVue(b, a.Text, indent, ctx)
	case "condition":
		writeConditionVue(b, a.Text, indent, ctx)
	case "loop":
		writeLoopVue(b, a.Text, indent, ctx, nil)
	case "query":
		// handled by onMounted
	default:
...
func writeDisplayVue(b *strings.Builder, text string, indent string, ctx *pageContext) {
	cleaned := text
	...
	if strings.Contains(lower, "button") {
		...
		if strings.Contains(lower, "create") || strings.Contains(lower, "new") {
			fmt.Fprintf(b, "%s<button class=\"fab\" @click=\"showForm = true\">+ %s</button>\n", indent, label)
		} else {
	...
	if strings.Contains(lower, "rich text") {
		expr := resolveFieldExpr(cleaned, ctx)
		if expr != "null" {
			fmt.Fprintf(b, "%s<div class=\"rich-text\" v-html=\"%s\"></div>\n", indent, expr)
		} else {
	...
	if strings.Contains(lower, "link") {
		...
		fmt.Fprintf(b, "%s<router-link to=\"%s\" class=\"link\">%s</router-link>\n", indent, target, label)
		return
	}
	...
	if strings.Contains(lower, "list of") || strings.Contains(lower, "list ") {
		fmt.Fprintf(b, "%s<!-- %s — rendered by v-for below -->\n", indent, text)
		return
	}
...
func writeInputVue(b *strings.Builder, text string, indent string, ctx *pageContext) {
	...
	if strings.Contains(lower, "form") {
		writeFormVue(b, text, indent, ctx)
		return
	}
...
func writeFormVue(b *strings.Builder, text string, indent string, ctx *pageContext) {
	lower := strings.ToLower(text)
...
func writeLoopVue(b *strings.Builder, text string, indent string, ctx *pageContext, fields []string) {
	dataVar := ctx.varName
	if dataVar == "" {
		dataVar = "data"
	}
	...
	fmt.Fprintf(b, "%s<div v-for=\"%s in %s\" :key=\"%s.id\" class=\"%s-item\">\n", indent, item, dataVar, item, modelClass)
	if len(fields) > 0 {
		for _, f := range fields {
			fieldExpr := item + "." + f
			fl := strings.ToLower(f)
...
func writeConditionVue(b *strings.Builder, text string, indent string, ctx *pageContext) {
	...
	if strings.Contains(lower, "while loading") || strings.Contains(lower, "is loading") {
		if strings.Contains(lower, "skeleton") {
			fmt.Fprintf(b, "%s<div v-if=\"loading\" class=\"skeleton-screen\">\n", indent)
			fmt.Fprintf(b, "%s  <div class=\"skeleton-item\" v-for=\"i in 3\" :key=\"i\"></div>\n", indent)
			fmt.Fprintf(b, "%s</div>\n", indent)
		} else {
			fmt.Fprintf(b, "%s<div v-if=\"loading\" class=\"loading-spinner\">\n", indent)
	...
	if strings.Contains(lower, "if no ") || strings.Contains(lower, "if there are no") {
		...
		fmt.Fprintf(b, "%s<div v-if=\"!loading && %s.length === 0\" class=\"empty-state\">%s</div>\n", indent, dataVar, message)
		return
	}
	...
	if strings.Contains(lower, "not logged in") || strings.Contains(lower, "is not logged in") {
		...
		fmt.Fprintf(b, "%s<div v-if=\"!isLoggedIn\" class=\"auth-prompt\">\n", indent)
		writeConditionButtonsVue(b, content, indent+"  ", ctx)
		fmt.Fprintf(b, "%s</div>\n", indent)
		return
	}
	if strings.Contains(lower, "is logged in") || strings.Contains(lower, "logged in") {
		content := extractConditionContent(text)
		fmt.Fprintf(b, "%s<div v-if=\"isLoggedIn\">\n", indent)
		writeConditionButtonsVue(b, content, indent+"  ", ctx)
		fmt.Fprintf(b, "%s</div>\n", indent)
		return
	}
	...
	if strings.Contains(lower, "succeed") || strings.Contains(lower, "success") {
		...
		fmt.Fprintf(b, "%s<div v-if=\"success\" class=\"alert alert-success\">{{ success || '%s' }}</div>\n", indent, message)
		return
	}

	// Error
	if strings.Contains(lower, "error") {
		fmt.Fprintf(b, "%s<div v-if=\"error\" class=\"alert alert-error\">{{ error }}</div>\n", indent)
		return
	}
...
func writeConditionButtonsVue(b *strings.Builder, content string, indent string, ctx *pageContext) {
	...
	if strings.Contains(lower, "login") && strings.Contains(lower, "signup") {
		fmt.Fprintf(b, "%s<button @click=\"router.push('/login')\">Log In</button>\n", indent)
		fmt.Fprintf(b, "%s<button @click=\"router.push('/sign-up')\">Sign Up</button>\n", indent)
		return
	}

	if strings.Contains(lower, "form") {
		writeFormVue(b, content, indent, ctx)
		return
	}
...
func writeInteractVue(b *strings.Builder, text string, indent string, ctx *pageContext) {
	lower := strings.ToLower(text)
	...
	if strings.Contains(lower, "clicking") || strings.Contains(lower, "click") {
		...
		if strings.Contains(lower, "opens a form") || strings.Contains(lower, "open a form") {
			fmt.Fprintf(b, "%s<button @click=\"showForm = true\">%s</button>\n", indent, capitalize(label))
			return
		}
		...
		if strings.Contains(lower, "update") || strings.Contains(lower, "save") {
			...
			handler := "/* TODO: save */"
			if ctx.hasSuccessState && ctx.hasErrorState {
				handler = "error = ''; success = 'Saved successfully'"
			} else if ctx.hasSuccessState {
				handler = "success = 'Saved successfully'"
			}
...
// ── Helpers (same text-parsing as React, framework-agnostic) ──
...
func resolveFieldExpr(text string, ctx *pageContext) string {
	...
	// Remove "truncated to X characters"
