...
func sanitizeAlertName(condition string) string {
	...
	for _, w := range words {
		...
		if w != "" {
			parts = append(parts, titleCase(w))
		}
	}
	if len(parts) == 0 {
		return "CustomAlert"
...
func conditionToPromQL(condition string) string {
	...
	default:
		return fmt.Sprintf("up == 0  # TODO: translate condition to PromQL: %s", condition)
	}
}
...
// safeVarName ensures a variable name doesn't start with a digit.
