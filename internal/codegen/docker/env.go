...
// generateEnvFile produces a .env with working defaults for local Docker development.
// Secrets use placeholder values so `docker compose up` works out of the box.
func generateEnvFile(app *ir.Application) string {
	var b strings.Builder

	b.WriteString("# Generated by Human compiler — local development defaults\n")
	b.WriteString("# Change JWT_SECRET before deploying to production!\n\n")
