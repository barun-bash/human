...
func generatePageTests(page *ir.Page, app *ir.Application) (string, int) {
	...
	componentName := page.Name + "Page"
	pageName := page.Name

	b.WriteString("/**\n")
	b.WriteString(" * @jest-environment jsdom\n")
	b.WriteString(" */\n")
	b.WriteString("// Generated by Human compiler — do not edit\n\n")
	b.WriteString("import '@testing-library/jest-dom';\n")
	...
	for _, action := range page.Content {
		switch action.Type {
		case "display":
			// Extract display text for assertion. The React page generator
			// renders display actions as a <div> with a class name, so use
			// a container query rather than getByText.
			displayText := extractDisplayText(action.Text)
			if displayText != "" {
				className := "show-" + strings.ToLower(strings.ReplaceAll(displayText, " ", "-"))
				fmt.Fprintf(&b, "  it('should display %s', () => {\n", escapeJSString(displayText))
				b.WriteString("    const { container } = renderPage();\n")
				fmt.Fprintf(&b, "    expect(container.querySelector('.%s')).toBeInTheDocument();\n", className)
				b.WriteString("  });\n\n")
				testCount++
			}
		...
		case "condition":
			...
			if strings.Contains(lower, "loading") {
				...
				b.WriteString("    const { container } = renderPage();\n")
				b.WriteString("    expect(container.querySelector('.loading-spinner')).toBeInTheDocument();\n")
				b.WriteString("  });\n\n")
				testCount++
			}
